...
const TableStudents = () => {
  ...
  if (studentLoading) {
    ...
    tableContent = studentData.students.map((user) => (
      <tr key={user.id}>
        ...
        <td className="px-6 py-4 whitespace-nowrap capitalize text-sm text-gray-800">
          {user?.nutrition[0]?.nutritionStatus?.information || "-"}
        </td>
        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
          {(() => {
            const isNormal =
              user?.nutrition?.[0]?.nutritionStatus?.information?.toLowerCase() ===
              "gizi normal";
            const isRecommended = recommendationData?.recomend?.some(
              (rec) => rec.student.id === user?.student?.id
            );
            ...
            return (
              <button
                ...
                onClick={() =>
                  addRecommendation(
                    { familyMemberId: user?.id, studentId: user?.student?.id },
                    accessToken
                  )
                }
              >
                {isRecommended ? "sudah di rekomendasikan" : "Rekomendasikan"}
              </button>
            );
          })()}
        </td>
