...
const Question = () => {
  ...
  const { values, handleSubmit, setFieldValue } = useFormik({
    ...
    onSubmit: async (values) => {
      await addResponseInstitution(selectedQuestion, values, accessToken);
    },
  });
      ...
      Promise.all(
        quesioner.map(async (q) => {
          try {
            ...
            return { id: q.id, answered: data.answered };
          } catch {
  ...
  React.useEffect(() => {
    if (quesioner && accessToken) {
      Promise.all(
        ...
        setResponseQuesioner(Obj);
        setLoadingQuesioner(
          Object.fromEntries(quesioner.map((q) => [q.id, false]))
        );
      });
    }
  }, [quesioner, accessToken, page, limit, keyword]);
              ...
              <div className="flex justify-end items-center gap-x-2 py-3 px-4 border-t border-gray-200">
                {question && currentIndex === question?.length - 1 && (
                  <button
                    ...
                    disabled={
                      values.answers.length !== question?.length ||
                      values.answers.some((a, idx) => {
                        ...
                        if (
                          q.type === "BOOLEAN" &&
                          typeof a.boolean_value !== "boolean"
                        )
                          return true;
                        if (
                          q.type === "SCALE" &&
  ...
  return (
    <>
      ...
      <div>
        {quesioner?.map((q) => {
          // Debugging: cek data sebelum TableResponse

          return answeredStatus[q.id] ? (
            <div key={q.id}>
              <h1 className="mt-10 mb-5 font-semibold">
                Riwayat Jawaban {q.title}
              </h1>
              <TableResponse
                TABLE_HEAD={TABLE_HEAD}
                content={"jawaban"}
                data={responeQuesioner[q.id]}
                dataKey={"answers"}
                page={responeQuesioner[q.id]?.page || 0}
                pages={responeQuesioner[q.id]?.totalPage || 0}
                rows={responeQuesioner[q.id]?.totalRows || 0}
                setPage={(p) => setPage((prev) => ({ ...prev, [q.id]: p }))}
                setQuery={(val) =>
                ...
                isLoading={loadingQuesioner[q.id]}
              >
                {(answer, index) => {
                  // PERUBAHAN: pastikan mapping id pakai Number()
                  const question = responeQuesioner[q.id]?.questions?.find(
                  ...
                  return (
                    <tr key={answer.id}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        ...
                        <FormEditResponse answer={answer} question={question} />
                      </td>
                    </tr>
                  );
                }}
